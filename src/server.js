...
const REQUIRED_SCHEMAS = ['reference', 'boundaries', 'infrastructure', 'landuse', 'legislation', 'users'];
...
const enablePostgis = async () => {
  ...
  try {
    await sequelize.query('CREATE EXTENSION IF NOT EXISTS postgis WITH SCHEMA public;');
    console.log('✓ PostGIS extension verified/enabled');
  } catch (err) {
    const message = err?.message || String(err);
    console.error('✗ PostGIS extension is required for GEOMETRY columns but could not be enabled.');
