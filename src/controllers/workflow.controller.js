// Workflow Controller - CRUD operations and map/metadata endpoints
...
const buildWorkflowWhereClause = (query = {}) => {
  ...
  if (typeFilter && typeFilter !== 'all') {
    if (!VALID_WORKFLOW_TYPES.includes(typeFilter)) {
      return { error: 'Invalid workflow type filter' };
    }
  ...
  if (statusFilter && statusFilter !== 'all') {
    if (!VALID_WORKFLOW_STATUSES.includes(statusFilter)) {
      return { error: 'Invalid workflow status filter' };
    }
...
const workflowController = {
  ...
  async getAll(req, res) {
    try {
      ...
      const { where, error } = buildWorkflowWhereClause(req.query);
      if (error) {
  ...
  async getMetadata(req, res) {
    try {
      const data = await getWorkflowMetadata();
      res.json({
        success: true,
        data,
      });
  ...
  async create(req, res) {
    let transaction;
    try {
      const normalizedPayload = normalizeWorkflowPayload(req.body);
    ...
    try {
      ...
      if (linkedLegislationCount > 0) {
        return res.status(409).json({
          ...
          linkedCount: linkedLegislationCount,
        });
      }
